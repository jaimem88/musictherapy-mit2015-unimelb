...
function mix(audioFiles,output){
  console.log("sox -m"+x+mixed+output+"mixed.wav");
  //MIX all files
  exec("sox -m"+x+mixed+output+"mixed.wav", puts);
  //Compress to MP3
  //sox -t wav -r 8000 -c 1 <wavfilename> -t mp3 <mp3filename>
  console.log("sox -t wav -r 88200 -c 1 "+mixed+output+"mixed.wav"+" -t mp3 "+mixed+output+"compMixed.mp3");
  exec("sox -t wav -r 88200 -c 1 "+mixed+output+"mixed.wav"+" -t mp3 "+mixed+output+"compMixed.mp3",puts);
}
